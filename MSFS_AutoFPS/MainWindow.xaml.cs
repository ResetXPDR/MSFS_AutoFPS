...
using static MSFS_AutoFPS.ServiceModel;
...
//            assemblyVersion = assemblyVersion.Substring(0, assemblyVersion.LastIndexOf('.')) + "";
            ...
            string latestAppVersionStr = GetFinalRedirect("https://github.com/ResetXPDR/MSFS_AutoFPS/releases/latest");
        ...
        protected void LoadSettings()
        {
            ...
            else
            {
                ...
                if (!serviceModel.UseExpertOptions) serviceModel.MinTLODExtra[(int)appProfiles.NonExpert] = !serviceModel.AutoTargetFPS;
            ...
            if (ServiceModel.TestVersion || serviceModel.LogSimValues) Logger.Log(LogLevel.Information, "MainWindow:LoadSettings", $"Expert: {serviceModel.UseExpertOptions} Mode: {serviceModel.ActiveGraphicsMode} ATgtFPS: {serviceModel.AutoTargetFPS} FltType: {(serviceModel.FlightTypeIFR ? "IFR" : "VFR")} TgtFPS: {txtTargetFPS.Text} TLODMethod: {serviceModel.TLODAutoMethod[serviceModel.activeProfile]} Tol: {txtFPSTolerance.Text}" + (!serviceModel.UseExpertOptions && serviceModel.MemoryAccess == null ? "" : $" TMin: {txtMinTLod.Text} TMin+: {(serviceModel.MinTLODExtra[serviceModel.activeProfile] ? "true" : "false")} TMax: {txtMaxTLod.Text} TMax+: {serviceModel.TLODExtraMtns[serviceModel.activeProfile]} MtnAltMin: {serviceModel.TLODExtraMtnsTriggerAlt[serviceModel.activeProfile]} TLODMaxExtra: {serviceModel.TLODExtraMtnsAmount[serviceModel.activeProfile]} TMax-: {serviceModel.MaxTLODHalvedNight[serviceModel.activeProfile]} OLODB: {serviceModel.OLODAtBase[serviceModel.activeProfile]} OLODT: {serviceModel.OLODAtTop[serviceModel.activeProfile]} OLODBAlt: {serviceModel.AltOLODBase[serviceModel.activeProfile]}") +$" CloudQ: {serviceModel.DecCloudQ[serviceModel.activeProfile]} CRecovT: {txtCloudRecoveryTLOD.Text} Pause: {serviceModel.PauseMSFSFocusLost} TLODBAlt: {serviceModel.AltTLODBase[serviceModel.activeProfile]} MaxDesRate {serviceModel.AvgDescentRate[serviceModel.activeProfile]} CustomOLOD: {serviceModel.CustomAutoOLOD[serviceModel.activeProfile]} OLODTAlt: {serviceModel.AltOLODTop[serviceModel.activeProfile]}");
        }
        ...
        protected void UpdateLiveValues()
        {
            ...
            if (serviceModel.IsSimRunning)
            {
                ...
                btnReset.ToolTip = "Reset TLOD, Clouds, Auto Target FPS and graphics mode detection to initial state.";
            }
            else
            {
                btnReset.Content = serviceModel.isMSFS2024 ? "24>20" : "20>24";
                btnReset.ToolTip = "Switch config settings from MSFS " + (serviceModel.isMSFS2024 ? "2024 to " : "2020 to ") + (serviceModel.isMSFS2024 ? "2020" : "2024");
            }
            ...
            else
            {
                ...
                lblSimFPS.Foreground = new SolidColorBrush(Colors.Black);
                if (serviceModel.OffsetSearchingActive) lblStatusMessage.Content = "Searching for MSFS settings location in memory";
     ...
     private void TextBox_SetSetting(TextBox sender)
        ...
        private void Hyperlink_RequestNavigate(object sender, RequestNavigateEventArgs e)
        {
            try
            {
                ...
                myProcess.StartInfo.FileName = "https://github.com/ResetXPDR/MSFS_AutoFPS/releases/latest";
                myProcess.Start();
                e.Handled = true;
            }
            catch (Exception ex)
