...
namespace MSFS_AutoFPS
{
    public class ServiceModel
    {
        ...
        public ServiceModel()
        {
            ...
            if (!TestVersion && (File.GetLastWriteTime(App.ConfigFile) > DateTime.Now.AddSeconds(-10) || File.GetLastWriteTime(App.ConfigFile2024) > DateTime.Now.AddSeconds(-10))) resetWindowPosition = true;
        ...
        public void ResetCloudsTLOD(bool ResetTLOD = true, bool ResetAll = false)
        {
            if (MemoryAccess != null && DefaultSettingsRead)
            {
                if (ResetAll) 
                {
                    ...
                    Logger.Log(MSFS_AutoFPS.LogLevel.Information, "ServiceModel:ResetCloudsTLOD", "Full reset of TLOD, clouds, graphics mode and Auto Target FPS states activated.");
                }
